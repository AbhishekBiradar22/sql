...
-- 4. Display all data from Employees table for all employees who was hired before January 1st, 1992.
	select * from employees where hire_date<'1992-1-1';
...
-- last name. Sort the query in descending order by salary
	select * from employees where last_name like '%l%' or last_name like '%j%' or last_name like '%h%'
...
-- order by the full name (Employees table)
	select concat(first_name, ',',last_name) as full_name,salary from employees where salary not between 
...
-- hire date is before January 1st, 1990.
	select * from employees where salary between 800 and 6000 and commission_pct is not null or
...
-- phone number equals 5. Sort the query in a descending order by the first name (Employees table)
    ...
    from employees where salary > 10000 or phone_number like '__5%' order by first_name desc ;

-- 18. Display the last name and salary for all employees who earn more than 12000 (Employees table).
	select last_name,salary from employees where salary > 12000;
    
-- 19. Display the last name and department number for all employees whose department number is equal to 
-- 50 or 80. Perform this exercise once by using the IN operator, once by using the OR operator.
	select last_name,department_id from employees where department_id in (50,80); -- using in 
    
    select last_name,department_id from employees where department_id like '%50%' or 
    department_id like'%80%'; -- using or
...
-- first letters of last name, concatenated with @oracle.com.
	 select first_name,last_name, concat(concat(substr(first_name,1,1),substr(last_name,1,3)),'@oracle.com') as
...
-- letters of last name, concatenated with @oracle.com.
	select first_name,last_name,concat(left(first_name,1),right(last_name,3),'@oracle.com') as
    email from employees;
...
-- 6. Phone numbers: 
-- a. For each employee, display the first name, last name, phone number and a new phone number using
-- the REPLACE function. in the new phone number replace all occurrences of 515 with 815
	select first_name,last_name,phone_number,replace(phone_number,'515','815')as new_phone from employees;
    
-- b. For each employee, display the first name, last name, phone number and a new phone number using
--  the REPLACE function. in the new phone number replace all prefixes of 515 with 815.
	select first_name,last_name,phone_number,replace(phone_number,'515','815')as new_phone from employees;

/* 7. For each employee, display : a. first name b. salary c. salary after a raise of 12% 
 ...
 select first_name,salary,salary+salary * 0.12 as after12,round(salary+salary * 0.12)as round,
 floor(salary+salary * 0.12) as afterfloor from employees;
...
select first_name,hire_date, date_sub(hire_date,interval 10 day) as afterminus,date_add(hire_date,interval 1 month) 
as afteraddmonth,datediff(sysdate(),hire_date) as diffrences  from employees;
...
-- the company, and number of years he works in the company.
	select first_name,last_name,hire_date,timestampdiff(month,hire_date,sysdate()) as monthsworks,
   timestampdiff(year,hire_date,sysdate()) as yearssworks from employees;
   
-- 10. For each employee, display the first name, hire date, and hire date plus one year
   select first_name,hire_date,date_add(hire_date, interval 1 year) as affteradd1year from employees;
...
-- and hire date rounded up to the nearest month
    ...
    CASE
    when date_format(hire_date,'%m') < 6 then date_format(hire_date,'%Y-%m-%d') 
    else
    date_format(date_add(hire_date ,interval 1 year),'%Y-%m-%d')
    end as rounded_year,
    case
    when date_format(hire_date,'%d') < 15 then date_format(hire_date,'%Y-%m-%d') 
    else
    date_format(date_add(hire_date ,interval 1 month),'%Y-%m-%d')
    end as rounded_month
    from employees;
-- 12. For each employee, display the first name, the day of his hire date, and the year of his hire date.
	select first_name,hire_date,dayname(hire_date)as days,year(hire_date)as years from employees;
...
-- format model: ‘DD/MM/YYYY’, for all employees whose last name begins with the letter D or K.
	select upper(last_name),date_format(hire_date,'%d-%m-%Y'),format(salary,'9,999.999')
    from employees;
...
-- If an employee doesn’t earn a commission, display 0 instead of NULL.
	select first_name,last_name,salary,commission_pct,ifnull(commission_pct,0) from employees ;
...
-- If an employee doesn’t earn a commission, display “No Commission” instead of NULL.
  select first_name,last_name,salary,commission_pct,ifnull(commission_pct,'no commission') from employees;
    
/*15. For each employee, display the first name, last name, salary, and a salary grade based on these 
conditions : a. if the salary is between 0 and 5000 – salary grade level is A 
...
-- column headings “FULL_NAME” and “CONTACT_DETAILS” respectively (Employees tables)
	select concat(last_name, ' ',first_name) as full_name,concat(phone_number,'--',email)
    as contact_details from employees;
...
-- 7.Create a query to display the unique salaries in Employees table
...
-- Problem Statements to be solve
...
-- b. Modify your query to display the results only for employees whose salary is greater the 10000.
	select job_id,count(employee_id),salary from employees where salary > 10000 group by job_id ;
    
-- c. Modify your query again, this time display the results only for jobs with more than 2 people.
	select job_id,count(employee_id) from employees  group by job_id having count(employee_id) >2;
...
-- departments whose number is in the range of 20 and 80, and their average salary is greater than 9000
	select department_id,avg(salary) from employees group by department_id having department_id between
    20 and 80 and avg(salary) > 9000;
    
-- Problem Statements to be solved using – SQL Sub-Queries

-- 1.Display the first name and salary for all employees who earn more than employee number103(Employeestable). 
...
select first_name,salary,department_id from employees where department_id =
...
select first_name,salary,department_id from employees where salary in
...
-- average salary, and also work at the same department as employee whose first name is Kevin
	...
	(select department_id from employees where first_name ='kevin');
...
-- a. For each employee, display the first name, last name, department number and department name. 
    select e.first_name,e.last_name,e.department_id,d.department_name from employees e inner join
...
-- departments 50 or 90.
	select e.first_name,e.last_name,e.department_id,d.department_name from employees e inner join
    departments d on e.department_id = d.department_id and e.department_id in(50,90);
...
-- a. For each department, display the department name, city, and state province.
	select d.department_name,l.city,l.state_province from departments d inner join locations l 
...
-- b. For each employee, display the full name, department name, city, and state province.
    ...
    employees e inner join departments d on e.department_id = d.department_id inner join
...
-- name contains the letter a
	...
	inner join locations l on d.location_id = l.location_id where e.last_name like '%a%';
...
-- b. Modify your query to display all departments including departments without any employees.
    ...
    employees e right outer join departments d on e.department_id=d.department_id;
...
-- a. For each employee, display the last name, and the manager’s last name. 
	select e.last_name,m.last_name as managerlname from employees e
	right outer join employees m on e.employee_id = m.manager_id;
...
-- same department as employee whose last name is “King”. 
	select e.first_name,e.last_name,e.department_id from employees e join employees d on 
    e.department_id = d.department_id and d.last_name ='king';

-- 7. Display the last name and salary for all employees who earn less than employee number 103
	select e.last_name,e.salary from employees e join employees d on
    e.salary < d.salary and d.employee_id=103;
